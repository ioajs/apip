const symbolName = Symbol('name'); // 参数名 key
const symbolWildcard = Symbol('*'); // 通配符 key
const symbolMiddleware = Symbol('middleware'); // 中间件 key
...
class Router {
   ...
   */
  add(method: string, path: string, ...middlewares: Function[]) {
